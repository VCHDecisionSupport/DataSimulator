...
ON tab.object_id = col.object_id;\n");
	fmt % database_name % database_name % database_name;
	std::wcout << fmt.str() << std::endl;
	return fmt.str();
}

meta::database ms_source_system::generate_meta_object(std::wstring database_name)
{
	...
	for (auto row : data)
	{
		...
		if (this_schema_name != prev_schema_name)
		{
			...
			if (std::find(database_ptr->begin(), database_ptr->end(), schema_ptr) == database_ptr->end())
			{
